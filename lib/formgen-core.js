 ...
 * - `ManualServiceDescriptionPlaceholder` → Service description
 ...
 * @property {string} [serviceDescription] - Description of the service (used in ManualServiceDescriptionPlaceholder)
 ...
 *   serviceDescription: "This service handles...",
...
export async function generateBundle(opts) {
  const { serviceName, serviceDescription, bpmnModeler, templates, now = new Date(), resolveDescriptions } = opts;
  ...
  for (const node of nodes) {
    ...
    for (let gwIdx = 0; gwIdx < gateways.length; gwIdx++) {
      const gw = gateways[gwIdx];
      const kind = gatewayType(gw);
      if (!firstKind) firstKind = kind;
      console.log(`[formgen]   Gateway ${gw.id}: type=${kind}, parallelIndex=${gwIdx + 1}`);
      const chooserPack = buildChooserForGateway(gw, kind, elementRegistry, gwIdx + 1);
    ...
    replacePlaceholdersInForm(
      template,
      { serviceName, serviceDescription, stepName, stepDescription, nextTaskText, refsHtml },
    ...
    // StartEvents get "000-start", others get ordered sequentially (e.g., "001-step-name", "002-step-name")
    const baseName = isStart
      ? "000-start"
      : `${String(nodeIndex).padStart(3, "0")}-${slug(stepName) || node.id}`;
    const formId = `${baseName}-${ts}`; // Unique ID includes timestamp
    
    nodeIndex++; // Increment for next non-start node
    ...
    for (let gwIdx = 0; gwIdx < gateways.length; gwIdx++) {
      const gw = gateways[gwIdx];
      enrichGatewayConditionsRecursive(gw, elementRegistry, moddle, modeling, 1, gwIdx + 1);
    }
...
function findParallelGateways(node) {
  ...
  for (const flow of (firstGw.outgoing || [])) {
    ...
    // Check if the target itself is a gateway
    const targetType = gatewayType(flow.target);
    if (targetType === "XOR" || targetType === "OR") {
      parallelGateways.push(flow.target);
      continue;
    }
    
    // Otherwise look for next gateway after the target
...
/**
 * Generates the variable key name for a given cascade level and parallel index.
 * 
 * This determines the form field variable name used in choosers and FEEL conditions:
 * - Parallel 1, Level 1: "nextTask"
 * - Parallel 1, Level 2: "nextTask_2"
 * - Parallel 2, Level 1: "nextTask_parallel_2"
 * - Parallel 2, Level 2: "nextTask_parallel_2_2"
 * etc.
 * 
 * These variables are used in FEEL conditions to route the process flow.
 * 
 * @param {number} level - Nesting level (1-based)
 * @param {number} parallelIndex - Parallel gateway index (1-based)
 * @returns {string} Variable name for this level and parallel index
 */
function varKeyForLevel(level, parallelIndex = 1) {
  if (parallelIndex === 1) {
    return level === 1 ? "nextTask" : `nextTask_${level}`;
  } else {
    return level === 1 ? `nextTask_parallel_${parallelIndex}` : `nextTask_parallel_${parallelIndex}_${level}`;
  }
}
 ...
 */
function buildCascadingChooser(gateway, registry, level = 1, parallelIndex = 1) {
  const key   = parallelIndex === 1 
    ? (level === 1 ? "nextTask" : `nextTask_${level}`)
    : (level === 1 ? `nextTask_parallel_${parallelIndex}` : `nextTask_parallel_${parallelIndex}_${level}`);
  ...
  for (const ch of choices) {
    ...
    if (childKind === "XOR") {
      ...
      let branchIndex = 0; // Track which parallel branch we're on
      ...
      for (const flow of (childGw.outgoing || [])) {
        ...
        const targetType = gatewayType(flow.target);
        if (targetType === "XOR" || targetType === "OR") {
          branchGw = flow.target;
        } else {
          // Otherwise look for next gateway after the target
          branchGw = firstGatewayAfter(flow.target);
        }
        
        if (branchGw) {
          const branchType = gatewayType(branchGw);
          branchIndex++; // Increment for each branch with a gateway
          
          // Use different parallelIndex for each branch to avoid key collisions
          const branchParallelIndex = parallelIndex + branchIndex - 1;
          
          if (branchType === "XOR") {
            // Build cascading chooser for this parallel XOR branch
            // Pass level + 1 and unique parallelIndex for this branch
            const branchChooser = buildCascadingChooser(branchGw, registry, level + 1, branchParallelIndex);
            parallelChooserComponents.push(...branchChooser.components);
          } else if (branchType === "OR") {
            // Build OR chooser for this parallel OR branch
            const branchChooser = buildOrChooser(branchGw, registry, branchParallelIndex);
            parallelChooserComponents.push(...branchChooser.components);
          }
        }
      }
 ...
 * - Parallel 1, Level 1: `= nextTask = "taskId"`
 * - Parallel 1, Level 2: `= nextTask_2 = "taskId"`
 * - Parallel 2, Level 1: `= nextTask_parallel_2 = "taskId"`
 * - Parallel 2, Level 2: `= nextTask_parallel_2_2 = "taskId"`
 * 
 ...
 * @param {number} level - Current nesting level (default: 1)
 * @param {number} parallelIndex - Parallel gateway index (default: 1)
 * 
 * @example
 ...
 */
function enrichGatewayConditionsRecursive(gateway, registry, moddle, modeling, level = 1, parallelIndex = 1) {
  const outs = gateway.outgoing || [];
  const kind = gatewayType(gateway);
  const key  = varKeyForLevel(level, parallelIndex); // Variable name for this level and parallel index

  // For non-XOR gateways, use flat mode (no cascading needed)
  if (kind !== "XOR") {
    enrichGatewayConditions(gateway, kind, registry, moddle, modeling, parallelIndex);
    return;
  }
  ...
  for (const sf of outs) {
    ...
    // If target is itself a gateway, recursively enrich it
    if (t.includes("Gateway")) {
      const tgtKind = gatewayType(tgt);
      
      // If target is a parallel gateway, process each branch with unique parallelIndex
      if (tgtKind === "AND") {
        let branchIndex = 0;
        for (const branchFlow of (tgt.outgoing || [])) {
          if (!branchFlow.target) continue;
          
          // Check if this branch has a gateway (directly or downstream)
          let branchGw = null;
          const branchTargetType = gatewayType(branchFlow.target);
          if (branchTargetType === "XOR" || branchTargetType === "OR") {
            branchGw = branchFlow.target;
          } else {
            branchGw = firstGatewayAfter(branchFlow.target);
          }
          
          if (branchGw) {
            branchIndex++;
            const branchParallelIndex = parallelIndex + branchIndex - 1;
            enrichGatewayConditionsRecursive(branchGw, registry, moddle, modeling, level + 1, branchParallelIndex);
          }
        }
      } else {
        // For non-parallel gateways, continue with same parallelIndex
        enrichGatewayConditionsRecursive(tgt, registry, moddle, modeling, level + 1, parallelIndex);
      }
    }
  }
}
 ...
 */
function buildOrChooser(gateway, registry, parallelIndex = 1) {
  const outs  = gateway.outgoing || [];
  const seen  = new Set(); // Prevent duplicates
  const items = [];
  
  // Variable name: "nextTasks" for first parallel, "nextTasks_parallel_2" for second, etc.
  const key = parallelIndex === 1 ? "nextTasks" : `nextTasks_parallel_${parallelIndex}`;
  ...
  return {
    ...
    components: [{
      type: "select",
      key: key, // Use variable name based on parallel index
 ...
 */
function buildChooserForGateway(gateway, kind, registry, parallelIndex = 1) {
  if (kind === "XOR") {
    return buildCascadingChooser(gateway, registry, 1, parallelIndex);
  }
  
  if (kind === "OR") {
    return buildOrChooser(gateway, registry, parallelIndex);
  }
 ...
 * @param {number} parallelIndex - Parallel gateway index (default: 1)
 */
function enrichGatewayConditions(gateway, kind, registry, moddle, modeling, parallelIndex = 1) {
  const outs = gateway.outgoing || [];
  ...
  // Generate variable names based on parallel index
  const xorKey = varKeyForLevel(1, parallelIndex); // e.g., "nextTask" or "nextTask_parallel_2"
  const orKey = parallelIndex === 1 ? "nextTasks" : `nextTasks_parallel_${parallelIndex}`;
  ...
  for (const sf of outs) {
    ...
    const expr =
      kind === "XOR"
        ? `= ${xorKey} = "${value}"`                    // Single selection
        : `= list contains(${orKey}, "${value}")`;      // Multiple selection
    const ce = moddle.create("bpmn:FormalExpression", { body: expr, language: "feel" });
    modeling.updateProperties(sf, { conditionExpression: ce });
  }
}
...
function replacePlaceholdersInForm(
  ...
  const swap = (s = "") => {
    ...
    out = swapWordCI(out, "ManualServiceDescriptionPlaceholder", context.serviceDescription ?? "");
...
function attachFormDefinition(node, formId, moddle, modeling) {
  ...
  if (bo.$type === "bpmn:UserTask") {
    ...
    // Create ioMapping with Task_Creation_Date input
    // This FEEL expression formats the current timestamp as DD.MM.YYYY HH:MM
    const taskCreationDateSource = 
      '=(if now().day < 10 then "0" else "") + string(now().day) + "." +\n' +
      '(if now().month < 10 then "0" else "") + string(now().month) + "." +\n' +
      'string(now().year) + " " +\n' +
      '(if now().hour < 10 then "0" else "") + string(now().hour) + ":" +\n' +
      '(if now().minute < 10 then "0" else "") + string(now().minute)';
    
    const inputParam = moddle.create("zeebe:Input", {
      source: taskCreationDateSource,
      target: "Task_Creation_Date"
    });
    ...
    values.push(formDef, userTaskExt, ioMapping);
  } else {
    values.push(formDef);
  }
