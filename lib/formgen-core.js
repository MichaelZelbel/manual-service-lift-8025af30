...
function buildCascadingChooser(gateway, registry, level = 1) {
  ...
  // top level select (no wrapper group)
  const components = [{
    type: "select",
    key,
    label,
  ...
  // for gateway options: add a child SELECT directly (no group, no box)
  for (const ch of choices) {
    if ((ch._type || "").includes("Gateway")) {
      const childGw = ch._el;
      // build next level
      const child = buildCascadingChooser(childGw, registry, level + 1);
      // patch each child component with conditional FEEL to show only when parent == this gateway id
  ...
  // leaf tasks at this immediate level (for placeholder text)
