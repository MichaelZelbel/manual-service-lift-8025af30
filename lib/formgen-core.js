...
function buildCascadingChooser(gateway, registry, level = 1) {
  ...
  const label = displayName(gateway) || "Please choose";
  ...
  for (const v of values) {
    if ((v._type || "").includes("Gateway")) {
      ...
      // Add child components directly with conditional, no group wrapper
      for (const childComponent of child.components) {
        components.push({
          ...childComponent,
          // Apply conditional to hide when parent doesn't match
          conditional: { hide: `= ${key} != "${v.value}"` },
          // Update label to use gateway name or fallback to "Please choose"
          label: childComponent.type === 'select' && !childComponent.label.includes('*') 
            ? (displayName(gw) || "Please choose")
  ...
  if (kind === "OR") {
    return {
      ...
      components: [{
        type: "checkbox-group",
        key: "nextTasks",
        label: displayName(gateway) || "Please choose",
        validate: { required: true },
