...
function buildCascadingChooser(gateway, registry, level = 1) {
  ...
  // Build immediate choices (tasks + gateways), de-duped
  const seen = new Set();
  const choices = [];
  ...
  // Top-level select for THIS XOR level (single-select)
  const components = [{
    type: "select",
    key,
    label,
  ...
  // For each gateway option, render its own chooser:
  //  - If child is XOR  -> recurse (single select)
  //  - If child is OR   -> multi-select (buildOrChooser)
  //  - If child is AND  -> no UI (all paths auto)
  for (const ch of choices) {
    ...
    if (childKind === "XOR") {
      ...
      for (const c of child.components) {
        c.conditional = { hide: `= ${key} != "${ch.value}"` }; // show only when parent equals this gateway
        components.push(c); // no wrapper group → no extra box/headline
      }
    } else if (childKind === "OR") {
      const orPack = buildOrChooser(childGw, registry); // returns { components: [ { type: "select", multiple: true, ... } ] }
      for (const c of orPack.components) {
        c.conditional = { hide: `= ${key} != "${ch.value}"` };
        components.push(c); // direct insert → multi-select appears, no wrapper
      }
    } else if (childKind === "AND") {
      // AND: nothing to render (all mandatory)
      continue;
    }
  }

  // Leaf tasks at THIS XOR level (for plain-text placeholders)
