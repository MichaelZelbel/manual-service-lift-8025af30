...
export async function generateBundle(opts) {
  ...
  for (const node of nodes) {
    ...
    // Find the first gateway immediately after this node (if any)
    const nextGw = firstGatewayAfter(node);
    const kind   = nextGw ? gatewayType(nextGw) : null; // "XOR", "OR", "AND", or null

    // Step 4: Build the chooser UI components based on gateway type
    // This generates dropdowns, checkboxes, or nothing depending on the gateway
    const chooserPack = nextGw
      ? buildChooserForGateway(nextGw, kind, elementRegistry)
      : { components: null, options: [] }; // No gateway = no chooser needed
    ...
    // Extract step name from BPMN element
    const stepName = displayName(node);

    // Step 6: Resolve step description and references (optional external callback)
    ...
    // Build HTML for references and text representation of next tasks
    ...
    // Step 7: Replace all plain-text placeholders in the form template
    ...
    // Step 8: Replace ReferencesPlaceholder token with proper HTML list (structural replacement)
    ...
    // Step 9: Generate unique form ID and filename
    ...
    // Ensure form schema is version 4 (Camunda Forms format)
    ensureSchemaV4(template);
    template.id = formId;

    // Step 10: Attach form binding to the BPMN element
    // This tells Camunda which form to use for this task
    attachFormDefinition(node, formId, moddle, modeling);

    // Step 11: Write FEEL conditions to gateway flows (recursive for XOR cascades)
  ...
  // Step 12: Save the enriched BPMN XML (now contains FEEL conditions and form bindings)
  const { xml: updatedBpmnXml } = await bpmnModeler.saveXML({ format: true });

  // Step 13: Create manifest listing all generated forms
  const manifest = {
    service: serviceName,
  ...
  // Step 14: Package everything into a ZIP archive
  const zip = new JSZip();
...
function buildCascadingChooser(gateway, registry, level = 1) {
  ...
  for (const ch of choices) {
    ...
    } // AND: nothing to render (all paths execute, no user choice needed)
  }
...
function attachFormDefinition(node, formId, moddle, modeling) {
  ...
  // Remove any existing FormDefinition to avoid duplicates
