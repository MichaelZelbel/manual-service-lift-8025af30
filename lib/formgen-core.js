...
function applyChooserPlaceholder(formJson, chooserComponentsOrNull) {
  ...
  // Safely read textual content from different static components
  const readText = (node) => {
    if (!node || typeof node !== "object") return "";
    // common fields used by form-js for static content
    return node.text ?? node.content ?? node.markdown ?? "";
  };
  ...
  function walk(list) {
    for (let i = 0; i < list.length; i++) {
      ...
      // Textual components that may carry the token (text, html, markdown)
      const textual = readText(node);
      if (textual && isNextToken(textual)) {
      ...
      // Container with children: if any child contains the token, replace the whole container
      if (node && Array.isArray(node.components)) {
        const hasTokenChild = node.components.some((c) => isNextToken(readText(c)));
        if (hasTokenChild) {
          if (!chooserComponentsOrNull) {
            list.splice(i, 1);
          } else {
