...
export async function generateBundle(opts) {
  ...
  if (!templates?.firstStep || !templates?.nextStep) {
    throw new Error(
      "generateBundle: templates.firstStep and templates.nextStep are required"
    );
  }
  ...
  // Gather StartEvents and UserTasks
  const nodes = elementRegistry.filter(
    (el) => el.type === "bpmn:StartEvent" || el.type === "bpmn:UserTask"
  );
...
function resolveToUserTask(el /*, registry */) {
  ...
  // Extend here if you insert intermediate tasks.
  return null;
}
