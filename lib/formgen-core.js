...
export async function generateBundle(opts) {
  ...
  for (const node of nodes) {
    ...
    // Fallback: if placeholder token is missing in template, but we built a chooser,
    // ensure the chooser is still rendered by prepending it to the form components.
    if (Array.isArray(chooserPack.components) && chooserPack.components.length) {
      const hasChooser = (function check(node) {
        if (!node) return false;
        if (Array.isArray(node)) return node.some(check);
        if (typeof node === "object") {
          const isChooser = (node.type === "select" || node.type === "radio" || node.type === "checkbox")
            && (node.key === "nextTask" || node.key === "nextTasks");
          if (isChooser) return true;
          if (Array.isArray(node.components) && node.components.length) return check(node.components);
        }
        return false;
      })(template);

      if (!hasChooser) {
        template.components = [ ...(chooserPack.components || []), ...((template.components) || []) ];
      }
    }
...
function firstGatewayAfter(node) {
  ...
  for (const sf of outgoing) {
    ...
    if (type.includes("Gateway")) {
      // Only return this gateway if it's a splitting gateway (multiple outgoing flows)
      const gwOutgoing = (t.outgoing || []).filter((flow) => flow && flow.target);
      if (gwOutgoing.length > 1) {
        return t;
      }
    }
  }
...
if (kind === "OR") {
  ...
  // collect unique leaf tasks reachable from each outgoing branch
  const options = outs
    .flatMap(sf => collectUserTasksFromGatewayTarget(sf.target, registry))
    .filter(t => {
      const id = t?.id;
    ...
    .map(t => ({ label: (t.businessObject?.name || "Next task"), value: t.id }));

  return {
    options,
    // IMPORTANT: single component, no wrapping group => no funky border/headline
    components: [{
      type: "select",
      key: "nextTasks",
      label: gateway.businessObject?.name || "Please choose",
      multiple: true,
      validate: { required: true },
...
function applyChooserPlaceholder(formJson, chooserComponentsOrNull) {
  ...
  function walk(list) {
    for (let i = 0; i < list.length; i++) {
      ...
      // Lone text token -> remove or insert chooser components (flattened)
      ...
      // Container that contains the token -> replace the WHOLE container with chooser components (flattened)
      if (node && Array.isArray(node.components)) {
        const idx = node.components.findIndex(c => c?.type === "text" && isNextToken(c.text));
        if (idx !== -1) {
          if (!chooserComponentsOrNull) {
            list.splice(i, 1); // remove chooser section entirely
          } else {
            replaceWithMany(list, i, chooserComponentsOrNull);
          }
          done = true;
