...
export async function generateBundle(opts) {
  ...
  // Gather StartEvents, UserTasks, and CallActivities
  ...
  console.debug("[formgen] nodes filtered:", nodes.map(n => n?.businessObject?.$type || n?.type || "?"));
  ...
  for (const node of nodes) {
    ...
    const resolved = resolveDescriptions
      ? (await resolveDescriptions(node))
      : { stepDescription: await getStepDescriptionFallback(node), references: [] };
    const stepDescription = resolved?.stepDescription || "";
    const references = resolved?.references || [];
    console.log(`[formgen] Node ${node.id} (${stepName}): ${references.length} references found`, references);
...
/**
 * Recursively collect all UserTask/CallActivity nodes reachable from a target,
 * walking through nested gateways.
 */
function collectUserTasksFromGatewayTarget(target, registry, seen = new Set()) {
  ...
  // stop at other element types (e.g., EndEvent, IntermediateEvent)
  return [];
}
...
function buildChooserForGateway(gateway, kind, registry) {
  ...
  // Recursively collect all reachable user tasks (handles nested gateways)
  const options = outs
...
function replacePlaceholdersInForm(
  ...
  // Format references as HTML list with clickable links
  const formatReferences = (refs) => {
    if (!Array.isArray(refs) || refs.length === 0) return "";
    return '<ul>' + refs.map(ref => `<li><a href="${ref.url}" target="_blank" rel="noopener noreferrer">${ref.name}</a></li>`).join('') + '</ul>';
  };

  const swap = (s = "") => {
    const nextTxt = forceBlankNextTask ? "" : context.nextTaskText || "";
    const refsText = formatReferences(context.references);
    let out = String(s)
      // ManualServiceName
      .replaceAll("{{ManualServiceNamePlaceholder}}", context.serviceName || "")
      .replace(/\bManualServiceNamePlaceholder\b/g, context.serviceName || "")
      .replace(/(?:\{\{\s*)?Manual\s*Service\s*Name\s*Placeholder(?:\s*\}\})?/g, context.serviceName || "")
      // ProcessStep
      .replaceAll("{{ProcessStepPlaceholder}}", context.stepName || "")
      .replace(/\bProcessStepPlaceholder\b/g, context.stepName || "")
      .replace(/(?:\{\{\s*)?Process\s*Step\s*Placeholder(?:\s*\}\})?/g, context.stepName || "")
      // ProcessDescription
      ...
      .replace(/(?:\{\{\s*)?Process\s*Description\s*Placeholder(?:\s*\}\})?/g, context.stepDescription || "")
      // References
      .replaceAll("{{ReferencesPlaceholder}}", refsText)
      .replace(/\bReferencesPlaceholder\b/g, refsText)
      .replace(/(?:\{\{\s*)?References\s*Placeholder(?:\s*\}\})?/g, refsText);
