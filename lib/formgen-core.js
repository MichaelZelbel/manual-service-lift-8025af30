...
export async function generateBundle(opts) {
  ...
  // Gather StartEvents and any Task-like node (UserTask, ServiceTask, ManualTask, etc.)
  const allEls = elementRegistry.getAll();
  const nodes = allEls.filter((el) => {
    const t = el?.businessObject?.$type || el?.type || "";
    return t === "bpmn:StartEvent" || (typeof t === "string" && t.startsWith("bpmn:") && t.endsWith("Task"));
  });
  ...
  for (const node of nodes) {
    ...
    const stepDescription = resolveDescriptions
      ? ((await resolveDescriptions(node))?.stepDescription || "")
      : await getStepDescriptionFallback(node); // fallback: BPMN documentation
