...
import { fetchReferencesForService } from '../src/integrations/mds/references.ts';
...
export async function generateBundle (opts) {
  ...
  // 0) Fetch service-wide references from MDS once
  const serviceKey = serviceName; // use stable service ID if available
  const refMap = await fetchReferencesForService(serviceKey);

  // 1) Find StartEvents + UserTasks
  const nodes = elementRegistry.filter(el =>
    el.type === 'bpmn:StartEvent' || el.type === 'bpmn:UserTask' || el.type === 'bpmn:CallActivity'
  );
  ...
  for (const node of nodes) {
    ...
    // References: prefer MDS refMap, fallback to BPMN $attrs
    const refsPrefer = refMap[node.id] || [];
    const refsFallback = getNodeReferencesFromMds(node);
    const referencesText = buildReferencesText(refsPrefer.length ? refsPrefer : refsFallback);
...
function firstUserTaskAfter(node, visited = new Set()) {
  ...
  // Check each outgoing path (for gateways, multiple paths)
  for (const sf of out) {
    const t = sf.target;
    if (!t) continue;
    ...
    // Found a UserTask or CallActivity
    if (type === 'bpmn:UserTask' || type === 'bpmn:CallActivity') return t;
    
    // Recurse through intermediate elements (gateways, events, etc.)
    const result = firstUserTaskAfter(t, visited);
...
function resolveToUserTask (el /*, registry */) {
  ...
  if (t === 'bpmn:UserTask' || t === 'bpmn:CallActivity') return el;
  // Prototype assumption: gateway connects directly to main step nodes.
  return null;
}
...
const REPLACE_FIELDS = new Set(['text', 'label', 'description', 'help', 'placeholder', 'title', 'content']);

function replacePlaceholdersInForm (formJson, context) {
  function swap (s = '') {
    const str = String(s);
    const r = (key) => new RegExp(`\\{\\{\\s*${key}\\s*\\}}`, 'g');

    return str
      // ManualServiceNamePlaceholder
      .replaceAll(r('ManualServiceNamePlaceholder'), context.serviceName || '')
      .replaceAll('ManualServiceNamePlaceholder', context.serviceName || '')
      // ProcessStepPlaceholder
      .replaceAll(r('ProcessStepPlaceholder'), context.stepName || '')
      .replaceAll('ProcessStepPlaceholder', context.stepName || '')
      // ProcessDescriptionPlaceholder
      .replaceAll(r('ProcessDescriptionPlaceholder'), context.stepDescription || '')
      .replaceAll('ProcessDescriptionPlaceholder', context.stepDescription || '')
      // NextTaskPlaceholder
      .replaceAll(r('NextTaskPlaceholder'), context.nextTaskText || '')
      .replaceAll('NextTaskPlaceholder', context.nextTaskText || '')
      // ReferencesPlaceholder
      .replaceAll(r('ReferencesPlaceholder'), context.referencesText || '')
  ...
  function walk (node) {
    ...
    for (const [k, v] of Object.entries(node)) {
      if (typeof v === 'string' && REPLACE_FIELDS.has(k)) {
        node[k] = swap(v);
...
function buildReferencesText(refs) {
  ...
  // Return HTML list with clickable links
  const items = refs
    .map(r => `<li><a href="${escapeHtml(r.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(r.name)}</a></li>`)
    .join('');
  return `<ul>${items}</ul>`;
}
